...
Frameworks ajudam você a *construir* o loop de um agente. Uma camada separada e em rápido crescimento (2026) foca em *executar e coordenar* agentes a longo prazo: **AgentsKit** como stack JS de ponta a ponta, **Hermes Agent** como um agente único persistente e auto-aprimorável, e **Paperclip** como um orquestrador via UI que opera um time de agentes existentes como uma empresa. Se o seu problema difícil é operação e não construção, veja [runtimes e orquestradores de agentes](/docs/agents/agent-runtimes).
