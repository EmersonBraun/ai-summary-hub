...
The Claude model lineup as of May 2026 follows a three-tier naming convention reflecting capability and cost trade-offs: **Opus** (highest quality, complex reasoning), **Sonnet** (balanced quality and speed), and **Haiku** (fastest and most cost-efficient).

| Model | API Model ID | Context | Max Output | Input ($/1M) | Output ($/1M) |
|-------|-------------|---------|------------|--------------|---------------|
...
> All Claude 3.x series models (Opus, Sonnet, Haiku) are fully superseded and should not be referenced as current. `claude-sonnet-4-20250514` and `claude-opus-4-20250514` retire **2026-06-15**.

From a platform perspective, Anthropic's API centers on the **Messages API** — a clean, purpose-built interface for multi-turn conversations. The platform includes tool use (Anthropic's term for function calling), extended thinking / adaptive thinking, prompt caching (reduces cost and latency for large repeated contexts), and batch processing. The Python SDK (`anthropic`) and TypeScript SDK are the primary client libraries. Claude models are also available through Amazon Bedrock, Google Cloud Vertex AI, and enterprise contracts with data residency options.
...
**Extended thinking** (Sonnet 4.6, Haiku 4.5) allows the model to reason at length before producing its final answer. When you set `thinking: {type: "enabled", budget_tokens: N}`, the model emits `thinking` content blocks containing its internal scratchpad — similar to chain-of-thought but native and structured.

**Adaptive thinking** (Opus 4.7, Opus 4.6, Sonnet 4.6) automatically scales reasoning depth to task complexity. Both modes significantly improve performance on math competitions, complex code, multi-step reasoning, and tasks requiring careful step-by-step analysis. The thinking tokens count toward the token budget but are visible in the response, giving you transparency into how the model arrived at its answer.

### Prompt caching

Prompt caching dramatically reduces cost and latency for workloads that repeatedly use large system prompts or document contexts. You mark prefix sections of your request with `cache_control: {type: "ephemeral"}`. Cache writes for a 5-minute TTL cost 1.25x base; cache writes for a 1-hour TTL cost 2x base; cache reads cost 0.1x base (90% discount). This is especially valuable for RAG pipelines (large context passed with every query), agent loops (large system prompts repeated every turn), and batch document processing.

### Long context (1M tokens)

Claude Opus 4.7 and Sonnet 4.6 support a 1M-token context window, and Haiku 4.5 supports 200K tokens. Long context enables entire codebases, legal documents, research papers, or full conversation histories to be processed in a single call without chunking. The **Batch API** provides a 50% discount on both input and output tokens for async workloads. An `output-300k-2026-03-24` beta header extends maximum output to 300K tokens.
...
|--------------------|--------------------------------------|
| You need a 1M context window to process long documents, codebases, or extended conversations without chunking | Your workload requires image generation, audio transcription, or text-to-speech — Claude is text/vision only; OpenAI covers audio |
...
| You want extended or adaptive thinking for deep reasoning tasks (math, complex code, multi-step analysis) | Your primary use case is high-volume embedding generation — Anthropic does not offer an embeddings API |
| Prompt caching will meaningfully reduce cost (large repeated contexts, agent system prompts) | You rely heavily on OpenAI-specific tooling (DALL-E, Whisper) that has no Anthropic equivalent |
| You are building tool use or computer use workflows and want a model well-calibrated for structured outputs | You need the absolute lowest cost-per-token at scale — Claude Haiku competes on price but open models can be cheaper |
...
| Context window | 1M (Opus 4.7, Sonnet 4.6); 200K (Haiku 4.5) | 1,050K (GPT-5.5 / GPT-5.4) | 1M (Gemini 2.5 Pro) |
| Reasoning / thinking | Adaptive thinking (Opus 4.7); Extended thinking (Sonnet 4.6, Haiku 4.5) | Reasoning modes: none → xhigh | Gemini 2.5 Pro thinking |
| Multimodal input | Text, image | Text, image, audio, video | Text, image, audio, video |
| Audio / speech | No | Yes (GPT Realtime 2) | Yes (Gemini) |
| Image generation | No | Yes (GPT Image 2) | Yes (Imagen 4) |
| Embeddings API | No | Yes | Yes |
| Open-weights | No | No | Gemma (partial) |
| Prompt caching | Yes (native, 90% read discount) | Context caching (limited) | Yes (Gemini) |
...
| Pros | Cons |
|------|------|
| Up to 1M context window — best-in-class for long documents | No audio, speech, or image generation APIs |
| Adaptive and extended thinking give transparent chain-of-thought for hard reasoning tasks | No embeddings API — you need a second provider for RAG |
...
| Computer use (beta) enables agentic control of desktop GUIs | Pricing can be higher than open-weights alternatives for simple tasks |
...
print(response.content[0].text)
# usage.cache_creation_input_tokens — tokens cached this call (full price × 1.25)
...
- [Claude models overview](https://platform.claude.com/docs/en/docs/about-claude/models) — Current model IDs, context windows, capability comparison, and deprecation schedule
- [Claude pricing](https://platform.claude.com/docs/en/docs/about-claude/pricing) — Per-token pricing, prompt caching rates, batch discount, and fast mode pricing
...
- [OpenAI](/docs/model-providers/openai) — GPT-5.x models, function calling, image and video generation
