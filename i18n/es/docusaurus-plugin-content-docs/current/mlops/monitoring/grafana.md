...
keywords: [Grafana, dashboards, visualización, Prometheus, InfluxDB, fuentes de datos, monitoreo de ML, alertas, paneles]
---
...
Grafana es una plataforma de análisis y visualización interactiva de código abierto que se conecta a una amplia gama de fuentes de datos — [Prometheus](/docs/mlops/monitoring/prometheus), InfluxDB, Elasticsearch, Loki, PostgreSQL, APIs de monitoreo nativas de la nube y docenas más — y renderiza los datos como dashboards interactivos y compartibles. No proporciona almacenamiento propio; es puramente una capa de consulta y visualización que se sitúa frente a la infraestructura de datos existente. Este diseño hace que Grafana sea complementario a todos los sistemas de almacenamiento de series temporales o logs en lugar de un reemplazo para cualquiera de ellos.

En los contextos de ML y MLOps, Grafana sirve como la interfaz de observabilidad unificada. Los científicos de datos y los ingenieros de ML la usan para rastrear métricas de rendimiento del modelo (precisión, F1, AUC) a medida que cambian con el tiempo, visualizar la latencia de predicción y el throughput junto al uso de recursos de infraestructura, y monitorear señales de calidad de datos como las puntuaciones de drift de características. Dado que Grafana soporta múltiples fuentes de datos simultáneamente, un único dashboard puede combinar métricas de Prometheus, logs de aplicaciones de Loki y KPIs de negocio de una base de datos SQL — dando una visión completa y contextualizada del comportamiento de un modelo en producción.

Grafana está disponible como distribución de código abierto auto-hospedada, como Grafana Cloud (una oferta SaaS gestionada) y como Grafana Enterprise con características empresariales adicionales. La distribución de código abierto es completamente funcional y es la opción más común para los equipos que ya operan Kubernetes o tienen flujos de trabajo de infraestructura como código, ya que los dashboards de Grafana, las configuraciones de fuentes de datos y las reglas de alertas pueden gestionarse como JSON o a través de providers de Terraform.

## Cómo funciona

### Configuración de fuentes de datos

Grafana se conecta a las fuentes de datos a través de plugins. Un plugin de fuente de datos traduce el modelo de consulta interno de Grafana al lenguaje de consulta nativo del backend (PromQL para Prometheus, SQL para bases de datos relacionales, Lucene para Elasticsearch, etc.) y devuelve datos en un formato normalizado. Las fuentes de datos se configuran en la interfaz de Grafana o a través de archivos de aprovisionamiento (YAML), lo que permite gestionar las configuraciones como código en un repositorio Git. La autenticación, TLS y la configuración de timeout son configurables por fuente de datos.

### Composición de dashboards y paneles

Un dashboard de Grafana es un documento JSON que contiene una lista ordenada de paneles. Cada panel define una consulta contra una fuente de datos, un tipo de visualización (serie temporal, gauge, gráfico de barras, tabla, heatmap, stat, etc.) y opciones de visualización (ejes, umbrales, leyendas, anulaciones). Los paneles pueden vincularse a otros dashboards, soportan variables (las variables de plantilla permiten que un único dashboard cambie entre entornos, versiones de modelos o servicios mediante un menú desplegable) y pueden referenciar anotaciones — eventos superpuestos en los gráficos de series temporales para marcar despliegues, ejecuciones de reentrenamiento o inicios de incidentes.
...
Las alertas de Grafana (introducidas en Grafana 8+) proporcionan reglas de alerta unificadas con múltiples fuentes de datos que evalúan las consultas de panel según un calendario y enrutan las alertas disparadas a puntos de contacto (Slack, PagerDuty, correo electrónico, webhooks). Las reglas de alerta se agrupan en políticas de notificación que determinan el comportamiento de enrutamiento, agrupación y silenciamiento. Las alertas de Grafana pueden coexistir con Prometheus Alertmanager o reemplazarlo completamente, dependiendo de las preferencias del equipo.

### Aprovisionamiento e infraestructura como código

Grafana soporta el aprovisionamiento declarativo de fuentes de datos, dashboards y reglas de alerta mediante archivos YAML y JSON cargados al inicio. Combinado con el provider Terraform de Grafana, toda la configuración de Grafana puede versionarse y desplegarse a través de pipelines de CI/CD — una capacidad crítica para los equipos que gestionan múltiples entornos o quieren una infraestructura de monitoreo reproducible.
...
|----------|------------|
| Necesitas dashboards interactivos y compartibles sobre Prometheus u otros datos de series temporales | Necesitas una interfaz de seguimiento de experimentos de ML completa (usa MLflow o W&B en su lugar) |
| Quieres correlacionar métricas de infraestructura con el rendimiento del modelo en una vista | Tu equipo no tiene fuente de datos de series temporales existente a la que conectar Grafana |
| Tienes múltiples fuentes de datos (Prometheus, SQL, Loki) que unificar en un dashboard | Un resumen textual o tabular simple es suficiente y un dashboard no agrega valor |
| Quieres gestionar dashboards como código a través de JSON o Terraform | Tu organización ya está estandarizada en una plataforma de observabilidad propietaria |
| Necesitas alertas que abarcan múltiples fuentes de datos | Necesitas almacenar o analizar logs de predicción sin procesar (Grafana consulta, no almacena) |
...
|-----------|---------|-----------|
| Rol principal | Visualización y dashboards | Recopilación, almacenamiento y alertas de métricas |
...
| Dashboard como código | La exportación JSON y el provider Terraform permiten flujos de trabajo GitOps | Los dashboards JSON son verbosos y difíciles de diferenciar manualmente |
...
| Biblioteca de visualización | Tipos de panel ricos y personalizables | Algunos tipos de gráficos avanzados requieren plugins o Grafana Enterprise |
| Alertas | Reglas de alerta unificadas y con múltiples fuentes de datos | Curva de aprendizaje para políticas de notificación y árboles de enrutamiento |
| Opción auto-hospedada | Control total, sin que los datos salgan de tu infraestructura | Requiere esfuerzo operacional: actualizaciones, copias de seguridad, gestión de plugins |
...
- [Mejores prácticas de dashboards de Grafana](https://grafana.com/docs/grafana/latest/dashboards/build-dashboards/best-practices/) — Guía oficial sobre la estructuración de dashboards efectivos, el uso de variables de plantilla y la organización de paneles.
- [Provider Terraform de Grafana](https://registry.terraform.io/providers/grafana/grafana/latest/docs) — Gestiona las fuentes de datos, dashboards y reglas de alerta de Grafana como infraestructura como código.
- [Awesome Grafana](https://github.com/monitoringartist/grafana-aws-cloudwatch-dashboards) — Colección curada por la comunidad de dashboards de Grafana prediseñados para pilas de infraestructura comunes.
- [Blog de Grafana Labs — Observabilidad de ML](https://grafana.com/blog/2021/08/09/how-to-monitor-machine-learning-models-with-grafana/) — Tutorial práctico para configurar dashboards de monitoreo de modelos de ML con Grafana y Prometheus.
