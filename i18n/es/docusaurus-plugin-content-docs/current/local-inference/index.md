...
You **obtain** model weights (por ej. GGUF, SafeTensors) from the Hub or a vendor. A **runtime** (Ollama, llama.cpp, vLLM, TFLite) **loads** the model onto CPU, GPU, or NPU and executes the forward pass. **Quantization** (INT8, INT4, GPTQ, AWQ) shrinks memory so larger models fit; **batching** and **KV cache** improve throughput when serving multiple requests. No network call to a cloud API—inference runs entidependen de the local machine or cluster.

## Casos de uso

Local inference fits when privacidad, latency, cost, or offline operation matters more than using the largest cloud model.
