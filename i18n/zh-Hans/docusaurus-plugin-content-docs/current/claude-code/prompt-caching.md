...
提示缓存是 Claude API 的一项功能，允许提示中重复出现的部分——系统提示、工具定义、文档上下文或长对话前缀——被处理一次并在后续多次 API 调用中复用。模型不是在每个请求上重新处理相同的 token 序列，而是从缓存中读取它们，从而降低延迟并减少缓存 token 的成本。在 Claude Code 中，提示缓存在后台自动运行，使长时间会话和重复工具使用更快、更便宜。

提示缓存背后的核心见解是，在编码会话中 API 调用之间变化的内容大多是少量的：一条新的用户消息、一个新的工具结果或一个更新的文件。大型稳定的部分——包含项目指令的系统提示、所有可用工具的定义以及积累的对话历史——在大多数轮次中保持不变。在每个请求上冗余处理这些稳定部分是浪费的。提示缓存通过存储处理过的键值状态并在提示前缀匹配时复用它们来消除这种浪费。

从开发者角度来看，提示缓存在 Claude Code 会话中基本是透明的——它在无需配置的情况下自动进行。了解它最重要的场景是直接在 Claude API 之上构建、设计长时间运行的代理循环，或解决意外的高延迟或成本问题。在这些情况下，了解如何构建提示以优化缓存利用率可以产生实质性节省：缓存的输入 token 以正常输入 token 成本的一小部分计费，而缓存命中会完全消除缓存部分的处理延迟。

## 工作原理

### 缓存控制标记

提示缓存使用明确的 `cache_control` 注解来标记在提示中应该创建缓存检查点的位置。当 API 处理内容块上带有 `{"type": "ephemeral"}` 缓存控制的请求时，它存储该块之前及包括该块在内的所有 token 的处理状态。在具有相同前缀的后续请求上，API 检测到缓存命中并跳过重新处理这些 token。一个提示最多可以同时有四个活跃的缓存检查点，允许开发者将系统提示与工具定义以及对话历史分别缓存。

### 缓存生命周期和失效

临时缓存的生存时间约为五分钟的不活动期。如果在该窗口内没有 API 调用引用缓存的前缀，缓存条目将被驱逐，必须在下次请求时重建。这意味着提示缓存对于高频用例最有效：请求每隔几秒到达的交互式编码会话、在快速连续中执行许多工具调用的代理循环，或使用共享系统提示处理许多文档的批处理管道。对于请求之间有分钟间隔的低频工作流，缓存可能会过期并且不提供任何好处。

### 缓存什么

最有价值的缓存候选是大型、稳定且频繁复用的内容。系统提示是最明显的候选：在 Claude Code 中，它包含来自 CLAUDE.md 的项目指令、工具定义和行为准则——通常是数千个在每个轮次中相同的 token。工具定义（所有可用工具的 JSON schema）是另一个强有力的候选。在 RAG 或文档密集型工作流中，在会话开始时加载的大型参考文档可以被缓存，这样后续关于这些文档的问题只产生新问题的边际成本，而不是重新读取文档的成本。

### 缓存命中检测和使用报告

API 响应包含一个 `usage` 对象，用于区分常规输入 token、缓存创建 token 和缓存读取 token。缓存创建 token 以基本费率的 1.25 倍计费（写入缓存的一次性成本）。缓存读取 token 以基本费率的 0.1 倍计费——比常规输入 token 节省 90%。监控这些字段让开发者可以衡量实际缓存效果：缓存读取 token 与总输入 token 的比率表明了有多少提示从缓存中提供。
...
| 使用场景 | 避免场景 |
|---|---|
| 系统提示很大（>1,000 token）且在许多请求中保持不变 | 每个请求的提示都有显著变化——没有稳定的前缀可缓存 |
| 正在运行具有许多顺序工具调用的代理循环 | 请求不频繁（轮次之间 >5 分钟）——缓存在可以复用之前就过期了 |
| 在会话开始时加载大型参考文档（规范、代码库） | 用例是单次请求，没有重复的上下文 |
| 希望在交互式会话中降低用户可见的第一响应延迟 | 正在测试或调试，希望每个请求的确定性 token 计数 |
| 正在优化高 API 调用量生产系统的成本 | 在低使用量时，构建 cache_control 标记的开销超过了节省 |
...
- [提示缓存 Cookbook](https://github.com/anthropics/anthropic-cookbook/blob/main/misc/prompt_caching.ipynb) — 包含成本计算和缓存命中测量的工作示例 Jupyter 笔记本。
- [Claude API 定价](https://www.anthropic.com/pricing) — 所有模型的输入、缓存创建和缓存读取 token 的当前费率。
- [Anthropic 使用量监控](https://console.anthropic.com/usage) — 用于检查每个 API 密钥的 token 使用量细分的控制台仪表板。

## 另请参阅

- [Claude Code 概述](/docs/claude-code)
- [上下文管理](/docs/claude-code/context-management)
- [思考模式与努力级别](/docs/claude-code/thinking-modes)
