...
模型上下文协议（MCP）是 Anthropic 开发的开放标准，定义了 AI 模型如何与外部数据源和工具通信。在 Claude Code 的上下文中，MCP 服务器是扩展 Claude 可用工具集的插件，超越了其内置能力（文件系统访问、shell 命令、git）。通过 MCP，Claude 可以查询数据库、调用外部 API、浏览网页、读取 Jira 工单、查询 Grafana 仪表板、与 GitHub 交互，以及 MCP 服务器实现的任何其他功能——所有这些都通过与内置工具相同的自然语言界面。

MCP 遵循客户端-服务器架构。Claude Code 充当 **MCP 客户端**：它从配置中发现可用的 MCP 服务器，在会话开始时连接到它们，并查询它们以获取公开的工具列表。每个 MCP 服务器公开一组**工具**（具有 JSON Schema 输入定义的函数，结构与内置工具相同）、可选**资源**（文档或数据库模式等只读数据源）和可选**提示**（可复用的提示模板）。当 Claude 决定调用 MCP 工具时，Claude Code 将调用路由到适当的服务器，执行操作，并将结果作为工具结果返回给模型。

MCP 相对于临时工具集成的主要好处是标准化。在 MCP 之前，每个 AI 助手都有自己的专有插件格式。MCP 定义了一种通用协议，因此写一次的 MCP 服务器可以与任何 MCP 兼容的客户端一起工作——Claude Code、Claude Desktop 或任何其他 MCP 客户端。这意味着可用集成的生态系统正在快速增长，构建新集成不需要了解 Claude 特定的内部机制。

## 工作原理

### MCP 服务器类型和传输

MCP 服务器有两种传输类型。**stdio 服务器**作为本地子进程运行：Claude Code 在会话开始时生成服务器进程，并通过 stdin/stdout 使用 JSON-RPC 进行通信。stdio 服务器是本地工具（数据库客户端、文件处理器、专业代码分析）中最常见的类型。**SSE（服务器发送事件）服务器**作为持久 HTTP 服务器运行，通过网络连接进行通信。SSE 服务器更适合远程服务、共享团队基础设施或需要在多个客户端连接之间维护状态的服务器。

### 配置和发现

MCP 服务器在 Claude Code 的设置文件中配置，对于全局配置通常在 `~/.claude/settings.json`，对于特定项目的服务器在 `.claude/settings.json`。每个服务器条目指定名称、传输类型以及启动或连接到它所需的命令（用于 stdio）或 URL（用于 SSE）。Claude Code 在会话开始时读取此配置，初始化与所有已配置服务器的连接，并获取其工具清单。所有已连接 MCP 服务器的工具与内置工具一起出现在模型的工具列表中——从模型的角度来看，没有区别。

### 工具调用流程

当 Claude 决定调用 MCP 工具时，Claude Code 充当中介：它将工具调用参数序列化为 JSON，通过配置的传输向 MCP 服务器发送 `tools/call` 请求，等待响应，反序列化结果，并将其作为 `tool_result` 块返回给模型。整个往返对模型是透明的——它只是看到工具结果，就像内置工具调用一样。MCP 服务器可以返回文本、图像或结构化数据，所有这些 Claude Code 都会适当地转发给模型。

### 身份验证和安全

MCP 服务器处理自己的身份验证。例如，GitHub MCP 服务器从服务器条目的 `env` 字段中配置的环境变量中读取 GitHub 个人访问令牌。Claude Code 将配置的环境传递给子进程，但不自己管理机密——对外部服务进行身份验证是 MCP 服务器的责任。因为 MCP 服务器以本地用户的权限执行代码，在使用第三方 MCP 服务器时应该谨慎：在将其添加到配置之前，先审查服务器的代码或信任发布者。
...
| 使用场景 | 避免场景 |
|---|---|
| 需要 Claude 与外部服务交互（GitHub、Jira、Slack、数据库） | 任务可以用内置工具完成（文件系统、shell）——无需增加复杂性 |
| 团队共享一套通用工具，希望有标准化的集成层 | 安全敏感的环境中，必须严格控制外部工具访问 |
| 希望给 Claude 访问私有数据源（内部 API、专有数据库） | 需要快速的一次性集成——通过 Bash 工具调用的 shell 脚本更简单 |
| 构建应该跨多个 AI 客户端工作的可复用工具 | 想要使用的 MCP 服务器来自不受信任的来源——先审查其代码 |
| 希望 Claude 实时访问系统状态（指标、日志、错误跟踪） | 外部服务有速率限制，Claude 的自主工具调用可能超出这些限制 |
...
- [MCP TypeScript SDK](https://github.com/modelcontextprotocol/typescript-sdk) — 在 TypeScript/JavaScript 中构建 MCP 服务器和客户端的官方 SDK。
- [MCP 服务器仓库](https://github.com/modelcontextprotocol/servers) — 参考 MCP 服务器的官方集合：文件系统、GitHub、PostgreSQL、Google Drive、Slack 等。
- [Claude Code MCP 集成指南](https://docs.anthropic.com/en/docs/claude-code/mcp) — 在会话中配置和使用 MCP 服务器的 Claude Code 特定指南。
- [MCP 规范](https://spec.modelcontextprotocol.io) — 用于实现自定义客户端或服务器的完整协议规范。
